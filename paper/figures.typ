...
#let bland_altman_rr() = {
  figure(
    canvas({
      ...
      draw.content((size_x + 0.3, 3.0), anchor: "mid-west", align(center)[Mean:#linebreak()#strfmt("{0:.2}", data_md)])
      draw.content((size_x, 4.2), anchor: "mid-west", align(
        center,
      )[$+1.96$SD:#linebreak()#{ strfmt("{0:.2}", data_md + 1.96 * data_sd) }])
      draw.content((size_x, 1.9), anchor: "mid-west", align(
        center,
      )[$-1.96$SD:#linebreak()#{ strfmt("{0:.2}", data_md - 1.96 * data_sd) }])
    }),
...
#let bland_altman_rr_talking() = {
  figure(
    ...
    caption: [RR estimated BPM plotted against Polar H10 reference BPM with O-sound (red), whisper (orange), and talking (purple) data],
  )
}
...
#let bland_altman_data() = {
  figure(
    canvas({
      ...
      draw.content((size_x + 0.3, 2.8), anchor: "mid-west", align(center)[Mean:#linebreak()#strfmt("{0:.2}", data_md)])
      draw.content((size_x, 4.2), anchor: "mid-west", align(
        center,
      )[$+1.96$SD:#linebreak()#{ strfmt("{0:.2}", data_md + 1.96 * data_sd) }])
      draw.content((size_x, 1.4), anchor: "mid-west", align(
        center,
      )[$-1.96$SD:#linebreak()#{ strfmt("{0:.2}", data_md - 1.96 * data_sd) }])
    }),
    caption: [Estimated HR BPM plotted against reference#linebreak()HR BPM],
  )
}
    ...
    canvas({
      ...
      plot.plot(
        ...
        x-min: x_min - buffer,
        {
          ...
          plot.add-hline(data_md_o, style: (stroke: (paint: purple, thickness: 1pt, dash: "solid")))
          plot.add-hline((data_md_o + 1.96 * data_sd_o), (data_md_o - 1.96 * data_sd_o), style: (
            stroke: (paint: purple, thickness: 1pt, dash: "dotted"),
          ))
        },
...
#let hr_combined_altman() = {
  figure(
    ...
    caption: [HR estimated BPM plotted against Polar H10 reference BPM with no sound (blue), O-sound (red), whisper (orange), and talking (purple) data],
  )
}
